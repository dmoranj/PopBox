...
var start = function start(done) {
  servers =[];
    ...
    var manageOrgs = require('./manageOrgs');
    var tags = require('./tagService');
    ...
    if (cluster.isMaster && numCPUs !== 0) {
        ...
        servers.forEach(function (server) {
            ...
            // Tag management URLs
            server.post('/tag', tags.createTag);
            server.get('/tag/:tagName', tags.getTag);
            server.delete('/tag/:tagName', tags.deleteTag);

            // Multitenant URLS
            server.get('/org/:id_org/trans/:id_trans',
                manageOrgs.appendOrg,
                manageOrgs.rewriteTrans,
                logic.transMeta);
            server.put('/org/:id_org/trans/:id_trans',
                manageOrgs.appendOrg,
                manageOrgs.rewriteTrans,
                logic.putTransMeta);
            server.post('/org/:id_org/trans',
                manageOrgs.appendOrg,
                manageOrgs.rewriteTrans,
                logic.postTransDelayed);
            server.get('/org/:id_org/queue/:id',
                manageOrgs.appendOrg,
                logic.getQueue);
            server.post('/org/:id_org/queue/:id/pop',
                manageOrgs.appendOrg,
                logic.popQueue);
            server.get('/org/:id_org/queue/:id/peek',
                manageOrgs.appendOrg,
                logic.peekQueue);
            server.post('/org/:id_org/queue/:id/subscribe',
                manageOrgs.appendOrg,
                logic.subscribeQueue);
        });
